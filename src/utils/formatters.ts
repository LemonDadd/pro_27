...
export const HIGHLIGHT_LABELS: Record<HighlightType, { label: string; emoji: string }> = {
  do: { label: "必做", emoji: "🎯" },
  eat: { label: "必吃", emoji: "🍽️" },
  see: { label: "必看", emoji: "👀" },
};
...
export function formatFlight(hours: number): string {
  if (hours < 1) return `${Math.round(hours * 60)} 分钟`;
  if (Number.isInteger(hours)) return `${hours} 小时`;
  return `${Math.floor(hours)} 小时 ${Math.round((hours % 1) * 60)} 分`;
}

export function getRandomDestinations<T>(arr: T[], count: number): T[] {
  const copy = [...arr];
  const result: T[] = [];
